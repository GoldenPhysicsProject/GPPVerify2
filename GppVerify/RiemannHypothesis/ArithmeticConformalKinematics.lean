...
/-- Arithmetic Hamiltonian energy associated to a positive norm/scale `a`: `E=log a`. -/
noncomputable def arithmeticEnergy (a : ℝ) : ℝ := Real.log a

/-- The corresponding real-time phase generated by the logarithmic Hamiltonian. -/
noncomputable def arithmeticTimePhase (tau a : ℝ) : ℂ :=
  Complex.exp ((((tau * arithmeticEnergy a) : ℝ) : ℂ) * Complex.I)

/-- Sampling the unitary principal-series character at logarithmic arithmetic energy
is exactly the phase generated by the logarithmic Hamiltonian.  This is the precise
kinematic bridge used by arithmetic quantum-statistical systems whose time evolution
acts by norm phases `N^{i tau}`. -/
theorem principal_character_eq_arithmeticTimePhase
    (tau a : ℝ) (ha : 0 < a) :
    logScaleCharacter (principalParameter tau) (arithmeticEnergy a) =
      arithmeticTimePhase tau a := by
  rw [principal_logScaleCharacter]
  simp [arithmeticTimePhase, arithmeticEnergy, mul_comm]
...
#print axioms GppArithmeticConformalKinematics.principal_character_eq_arithmeticTimePhase
